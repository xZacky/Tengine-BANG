                     ...
                     const float* scale, int loop_count, int num_thread, int affinity)
{
    ...
    int rtt = add_context_device(bang_context, "BANG");
    if (0 > rtt)
    {
        fprintf(stderr, " add_context_device Cambricon BANG DEVICE failed.\n");
        return -1;
    }

    /* create graph, load tengine model xxx.tmfile */
    graph_t graph = create_graph(bang_context, "tengine", model_file);
    if (NULL == graph)
    {
